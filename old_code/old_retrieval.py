...
# Load TF-IDF vectorizer for hybrid search
vectorizer = joblib.load(VECTORIZER_PATH)
print(f"Loaded TF-IDF vectorizer from {VECTORIZER_PATH}")

# Initialize reranker (lazy loading - will only be created when needed)
...
def extract_query_entities(query):
    ...
    # Find Điều references
    dieu_patterns = [r'điều\s+(\d+)', r'dieu\s+(\d+)']
    dieu_matches = []
    for pattern in dieu_patterns:
        matches = re.findall(pattern, query)
        dieu_matches.extend(matches)
    
    if dieu_matches:
        entities['dieu'] = list(set(dieu_matches))
    
    # Find Thông tư references
    tt_patterns = [r'thông\s*tư\s+(?:số\s+)?(\d+)', r'tt\s*(\d+)']
    tt_matches = []
    for pattern in tt_patterns:
        matches = re.findall(pattern, query)
    ...
    if tt_matches:
        entities['thong_tu'] = [f"tt{num}" for num in set(tt_matches)]
...
#     if 'dieu' not in entities and 'thong_tu' not in entities and 'nghi_dinh' not in entities:
#         return results  # No filtering needed if no specific legal entities
...
#             reference_id = reference_id.lower() if isinstance(reference_id, str) else ''
...
#                     is_match = True
#                 elif 'nghi_dinh' in entities and any(nd[2:] in doc_name for nd in entities['nghi_dinh']):
#                     is_match = True
            
#             # Also check reference_id for matches
#             if not is_match and 'dieu' in entities and reference_id:
#                 for dieu in entities['dieu']:
#                     if f"_d{dieu}" in reference_id or f"_d{dieu.zfill(2)}" in reference_id:
#                         is_match = True
#                         break
...
#     print(f"Entity filtering matched {len(filtered)} out of {len(results)} results")
...
def basic_entity_filter(results, entities):
    ...
    for result in results:
        try:
            payload = result.payload
            headline = payload.get('article_headline', '').lower()
            doc_name = payload.get('document_name', '').lower()
            reference_id = payload.get('reference_id', '').lower()
...
#     """
#     True hybrid search combining dense and sparse vectors using RRF fusion.
...
#     """
#     # Create sparse vector from TF-IDF
#     sparse_matrix = vectorizer.transform([text_query])
#     feature_names = vectorizer.get_feature_names_out()
#     print("TF-IDF terms:", [(feature_names[i], sparse_matrix[0, i]) for i in sparse_matrix.indices])
...
#         ).tolist()
#     else:
#         dense_vector = query_embedding_huggingface([text_query], model)[0]

#     try:
#         print("Performing hybrid search with both dense and sparse vectors...")
#         # Use query_points with prefetch for both vectors and RRF fusion
...
#         print(f"Hybrid search failed: {e1}")
#         try:
#             # Fall back to dense vector only
#             print("Falling back to dense vector search only...")
...
def search_hybrid_similar(text_query, collection_name, top_k=10, model=None):
    ...
    # Extract entities for post-processing
    ...
    if hasattr(model, 'encode'):
        ...
        dense_vector = query_embedding_huggingface([text_query], model)[0].tolist()  # Ensure it's a list
    ...
    # If we have entities, attempt to filter and boost results
    if entities and not results_df.empty:
        # Convert DataFrame back to objects for filtering
        ...
        filtered_objects = basic_entity_filter(objects, entities)
        ...
        if filtered_objects:
            ...
            for obj in filtered_objects:
                ...
                item.update(obj.payload)
                filtered_data.append(item)
            
            results_df = pd.DataFrame(filtered_data)
